...
public class TrackSession extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * If a user gets a session for the first time, you are given a brand new session
		 * If a session already exists, you are given an already created session
		 */
		...
		if(session==null) {
			out.print("<h1>WHO EVEN ARE YOU? GET OUTTA HERE</h1>");
		}else {
			...
			out.println("<tr><th>CREATION ID</th><td>" + new Date(session.getCreationTime()) + "</td></tr>");
